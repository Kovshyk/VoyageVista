...
<section class="page-head contact-head display-table" style="background-image: url(/frontend/img/backgrounds/about_bg.jpg)">
    <div class="display-table__cell">
        <div class="container">
            <div class="contact-head__content">
                <img src="/frontend/img/logo/logo.svg" class="contact-head__logo" alt="WonderWood logo" title="WonderWood logo">
                <span class="heading-1 --modifier">Voyage<span>Vista</span></span>
            </div>
        </div>
    </div>
</section>
<div class="container">
    <?= $this->element('breadcrumbs', [
        ...
        'active' => $page->getContentField(['contacts', 'title'])
    ]); ?>
</div>
        ...
        <div class="row m-row">
            <div class="column col-md-6 col-xs-12">
                <div class="contact-form">
                    <h2 class="heading-2 text-center contact-form__title"><?= $page->getContentField(['feedback', 'title']) ?></h2>
                    <p class="app-txt contact-form__txt"><?= $page->getContentField(['feedback', 'description']) ?></p>
                    <form method="post" action="<?= $this->getNiceUrl('/sendForm/formContact'); ?>"  id="contact-form">
                        <input type="hidden" name="g-recaptcha-response" value="" id="contact-form-token">

                        <div class="form-group">
                            <label class="form-label" for="user_name"><?= $page->getContentField(['feedback', 'field1', 'title']) ?></label>
                            <input type="text" name="name" id="user_name" class="form-input" placeholder="<?= $page->getContentField(['feedback', 'field1', 'placeholder']) ?>">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="user_phone"><?= $page->getContentField(['feedback', 'field2', 'title']) ?></label>
                            <input type="text" name="phone" id="user_phone" class="form-input phone-mask"
                                   placeholder="<?= $page->getContentField(['feedback', 'field2', 'placeholder']) ?>">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="user_message"><?= $page->getContentField(['feedback', 'field3', 'title']) ?></label>
                            <textarea type="text" name="message" id="user_message" class="form-textarea"
                                      placeholder="<?= $page->getContentField(['feedback', 'field3', 'placeholder']) ?>"></textarea>
                        </div>
                        <div class="form-group row m-row">
                            <div class="column col-md-6 col-sm-6 col-xs-12 mb-15">
                                <button type="button" id="contact-form-submit" class="btn btn-secondary"><?= $page->getContentField(['feedback', 'button']) ?></button>
                            </div>
                            <div class="column col-md-6 col-sm-6 col-xs-12">
                                <span class="app-txt"><?= $page->getContentField(['feedback', 'buttonDescription']) ?></span>
                            </div>
                        </div>
                    </form>
                </div>
            ...
            <div class="column col-md-5 medium-offset-1 col-xs-12">
                <div class="contact-content-block">
                    <h1 class="heading-2 contact-content-block__title"><?= $page->getContentField(['contacts', 'title']) ?></h1>
                    <div class="row m-row ">
                        <div class="column col-lg-6 col-md-12 col-xs-12">
                            <h2 class="heading-3 contact-content-block__subtitle"><?= $page->getContentField(['contacts', 'phone']) ?></h2>
                            <ul class="contact-content-block__list">
                                <?php if(!empty($configs['phones'])){
                                    foreach ($configs['phones'] as $key => $phone) { ?>
                                        <li class="contact-content-block__item">
                                            <a href="tel:<?= $phone ?>" class="contact-content-block__link" onclick="sendGoogleTel();">
                                                <?php if($this->detectMobileOperator($phone) == 'vodafone' ){  ?>
                                                    <img src="/frontend/img/vodafone.png" class="contact-content-block__link-image" alt="vodafone">
                                                <?php }  ?>
                                                <?php if($this->detectMobileOperator($phone) == 'lifecell' ){  ?>
                                                    <img src="/frontend/img/lifecell.png" class="contact-content-block__link-image" alt="lifecell">
                                                <?php }  ?>
                                                <?php if($this->detectMobileOperator($phone) == 'kyivstar' ){  ?>
                                                    <img src="/frontend/img/kyivstar.png" class="contact-content-block__link-image" alt="kyivstar">
                                                <?php }  ?>
                                                <?= $phone ?>
                                            </a>
                                        </li>
                                ...
                                <?php if(!empty($configs['viber'][0])) { ?>
                                    <li class="contact-content-block__item">
                                        <a href="tel:<?= $this->preparePhoneNumber($configs['viber'][0]) ?>" onclick="sendGoogleTel();" style="margin-top: 10px;" class="contact-content-block__link">
                                ...
                                <?php if(!empty($configs['whatsapp'][0])) { ?>
                                    <li class="contact-content-block__item">
                                        <a href="tel:<?= $this->preparePhoneNumber($configs['whatsapp'][0]) ?>" onclick="sendGoogleTel();" style="margin-top: 10px;" class="contact-content-block__link">
                        ...
                        <div class="column col-lg-6 col-md-12 col-xs-12">
                            <h2 class="heading-3 contact-content-block__subtitle"><?= $page->getContentField(['contacts', 'email']) ?></h2>
                            <ul class="contact-content-block__list">
                                <li class="contact-content-block__item">
                                    <a href="mailto:<?= (!empty(!empty($configs['email'][0]))) ? $configs['email'][0] : '' ?>" class="contact-content-block__link">
                                        <svg version="1.1" x="10px" y="10px" width="20" height="20" viewBox="0 0 612 612" style="enable-background:new 0 0 612 612;" xml:space="preserve">
                            ...
                            <h2 class="heading-3 contact-content-block__subtitle"><?= $page->getContentField(['contacts', 'socials']) ?></h2>
                            <div>
                                <?php if(!empty($configs['social_instagram'][0])){ ?>
                                    <a href="<?= $configs['social_instagram'][0] ?>" class="contact-content-block__social icon-instagram" rel="nofollow" target="_blank"></a>
                                <?php } ?>
                                <?php if(!empty($configs['social_youtube'][0])){ ?>
                                    <a href="<?= $configs['social_youtube'][0] ?>" class="contact-content-block__social icon-youtube" rel="nofollow" target="_blank"></a>
                                <?php } ?>
                                <?php if(!empty($configs['social_facebook'][0])){ ?>
                                    <a href="<?= $configs['social_facebook'][0] ?>" class="contact-content-block__social icon-facebook" rel="nofollow" target="_blank"></a>
                                <?php } ?>
                            </div>
                        </div>
                    </div>
